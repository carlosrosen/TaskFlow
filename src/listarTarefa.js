...
async function carregarTarefas() {
  ...
  userTasks.forEach((element) => {
    ...
    priorityCircle.style.backgroundColor = definirCorPrioridade(
      element["nivel_prioridade"],
    );
...
async function deletarTarefa(id) {
  ...
  const tarefas_atualizada = tarefas[user.value].filter(
    (task) => task["id"] !== id,
  );
...
async function logout() {
  ...
  const urlAtual = window.location.href.split("/");
  window.location.replace(
    urlAtual[0] + "/" + urlAtual[1] + "/" + urlAtual[2] + "/login.html",
  );
}
...
function alterarPagina(name) {
  const urlAtual = window.location.href.split("/");
  window.location.replace(
    urlAtual[0] + "/" + urlAtual[1] + "/" + urlAtual[2] + `/${name}`,
  );
}
